...
Kernel::Initialize(int quantum) {
    ...
    alarm = new Alarm(quantum); // start up time slicing
...
//invalidate physical memory pages after termination
void Kernel::releaseMem() {
    ListIterator<TranslationEntry*> iter(&entryList);
    for (; !iter.IsDone(); iter.Next()) {
        if (iter.Item()->pid == currentThread->getPID() && iter.Item()->physicalPage!=-1) {
            freeMap->Clear(iter.Item()->physicalPage);
            iter.Item()->physicalPage = -1;
            iter.Item()->valid = false;
        }
    }
